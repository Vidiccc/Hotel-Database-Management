...
//create SQL query
if($type == 'customer') {
	$sql = "delete from " .$type. " where customer_id = '" .$id. "'";
	$sql2 = "select * from " .$type;
}
elseif ($type == 'order_list') {
	$sql = "delete from ".$type." where order_id = '".$id. "'";
	$sql2 = "select * from " .$type;
}
elseif ($type == 'product') {
	$sql = "delete from ".$type." where product_id = '".$id."'";
	$sql2 = "select * from " .$type;
}
elseif ($type == 'branch') {
	$sql = "delete from ".$type." where branch_id = '".$id."'";
	$sql2 = "select * from " .$type;
}
elseif ($type == 'contains') {
	$sql = "delete from ".$type." where order_id = '".$id."' and product_id = '" 
	.$att2."'";
	$sql2 = "select * from " .$type;
}
elseif ($type == 'offers') {
	$sql = "delete from ".$type." where product_id = ".$id." and branch_id = " 
	.$att2."";
	$sql2 = "select * from " .$type;
}
...
if (!$db_handler) {
  ...
  echo "Please check again your id, use show or search function to check it!.<br>";
}
else {
	echo "<p>Record delete successfully! </p>" ;
}
